...
export default function Home() {
  ...
  if (records?.length === 0) {
    ...
    records?.forEach(function (v, i) {
      ...
      if (geshi === 'mp4') {
        html += '<li><i></i>';
        html += `< a href=" 'auctionDetails.html?id=' + v.id : 'artworkDetails.html?id=' + v.id}" class="artwork-mask videoPlay" ><div class="artwork-mask-wrap"></div>`;
  ...
  return (
    <div className="bzy-e center-85">
      <ul dangerouslySetInnerHTML={{ __html: html }} className={["bzy-e-list", records.length <= 3 ? 'bzy-e-list-flex' : '', records.length === 0 ? 'paddTop100' : '']}>
      </ul>
    </div>
  )
}